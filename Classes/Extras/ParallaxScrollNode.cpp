...
void ParallaxScrollNode::updateWithVelocity(CCPoint vel, float dt) {
	...
	CCARRAY_FOREACH(_scrollOffsets, object) {
		...
		CCPoint offset = ccpCompMult(cocos2d::ccpMult(totalVel, dt),
		...
		if ((offset.x < 0
			...
			child->setPosition(
					ccpAdd(child->getPosition(),
							ccp(
									-SIGN(offset.x) * fabs(scrollOffset->getScrollOffset().x),
									0)));
		}
		...
		if ((offset.y < 0
			...
			child->setPosition(
					ccpAdd(child->getPosition(),
							ccp(0,
									-SIGN(offset.y) * fabs(scrollOffset->getScrollOffset().y))));
		}
	}
}
...
void ParallaxScrollNode::updateWithXPosition(float x, float dt) {
	...
	CCARRAY_FOREACH(_scrollOffsets, object) {
		...
		child->setPosition(
				ccp(scrollOffset->getOrigPosition().x + offset,
						child->getPosition().y));
	}
}

void ParallaxScrollNode::addChild(CCSprite *node, int z, CCPoint r, CCPoint p,
		CCPoint so, CCPoint v) {
	node->setAnchorPoint(ccp(0, 0));

	ParallaxScrollOffset *obj = ParallaxScrollOffset::scrollWithNode(node, r, p,
			so, v);
...
void ParallaxScrollNode::addInfiniteScrollWithObjects(CCArray *objects, int z,
	...
	CCARRAY_FOREACH(objects, object) {
		sprite = (CCSprite*) object;
		this->addChild(sprite, z, ratio, currPos, ccp(totalWidth, totalHeight),
				relVel);
		CCPoint nextPosOffset = ccp(
				dir.x * (sprite->getContentSize().width + padding.x),
				dir.y * (sprite->getContentSize().height + padding.y));
...
void ParallaxScrollNode::addInfiniteScrollXWithZ(int z, CCPoint ratio,
		CCPoint pos, CCSprite* firstObject, ...) {
	va_list args;
	va_start(args, firstObject);

	CCArray *argArray = new CCArray();
	for (CCSprite *arg = firstObject; arg != NULL;
			arg = va_arg(args, CCSprite*)) {

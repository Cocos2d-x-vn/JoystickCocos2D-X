...
bool SneakyJoystick::initWithRect(CCRect rect) {
	bool pRet = false;
	//if(CCSprite::init()){
	stickPosition = CCPoint((float) (0), (float) (0));
	degrees = 0.0f;
	velocity = CCPoint((float) (0), (float) (0));
	autoCenter = true;
	isDPad = false;
	hasDeadzone = false;
...
void SneakyJoystick::updateVelocity(CCPoint point) {
	...
	velocity = CCPoint(dx / joystickRadius, dy / joystickRadius);
	degrees = angle * SJ_RAD2DEG;

	// Update the thumb's position
	stickPosition = CCPoint(dx, dy);
}
...
void SneakyJoystick::ccTouchEnded(CCTouch *touch, CCEvent *event) {
	CCPoint location = CCPoint((float) (0), (float) (0));
	if (!autoCenter) {
		CCPoint location = CCDirector::sharedDirector()->convertToGL(
				touch->getLocationInView());
		location = this->convertToNodeSpace(location);
